...
2. **Given** user has an account, **When** user enters correct credentials and logs in, **Then** user receives valid JWT token and accesses their dashboard
...
3. **Given** user is logged in, **When** user makes an API request with invalid JWT, **Then** system rejects the request
...
- How does the system handle expired JWT tokens during active usage?
...
- **FR-002**: System MUST allow users to log in with their credentials and receive a JWT token with 24-hour expiration
...
- **FR-010**: System MUST reject requests with invalid or expired JWT tokens
...
- **FR-013**: System MUST validate that user_id in request matches user_id in JWT token
...
- **FR-016**: System MUST enforce single session per user - logging in on a new device/session invalidates previous sessions
...
- **User**: Represents an authenticated user of the system, identified by unique email address and secured by password. Contains JWT token for authentication.
...
- **JWT Token**: Authentication token issued upon successful login, containing user identity and expiration information, validated on all protected operations.
...
- Q: What should be the default expiration time for JWT tokens issued upon successful authentication? → A: 24 hours (1 day)
...
- Q: Should the system allow users to be logged in on multiple devices/sessions simultaneously? → A: No, single session only - Enhanced security by restricting to one session
